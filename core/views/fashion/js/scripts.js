...
if (document.querySelector('.shop-page')) {
  var minPrice = parseInt($('.min-price-start').text()),
      maxPrice = parseInt($('.max-price-start').text());
  $('.range__line').slider({
      min: minPrice,
      max: maxPrice,
    ...
    stop: function(event, ui) {
      ...
      $('.max-price').text($('.range__line').slider('values', 1) + ' руб.');
        $('input[name=min_price]').val($('.range__line').slider('values', 0))
        $('input[name=max_price]').val($('.range__line').slider('values', 1))
    },
